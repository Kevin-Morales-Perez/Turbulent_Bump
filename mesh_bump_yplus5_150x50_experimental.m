...
domHgt=4;                            %Domain Height (m)
domLgt=4.5;                          %Domain Length (m)
...
if (distPlat+ bumpLgt)<domLgt
    ...
    est_wall_d=8.5e-6;%Estimated wall distance (Height of the first cell)
    ...
    %  domain length
    fpRatio = 0.5;%665;%bumpLgt/domLgt;% Now the plate should contain half o f the 
    %total cells in X
    ...
    dx_1=(est_wall_d)*300;%Lenght of the first cell at the leading and trailing edge
    ...
    grthF=1.297;% Geometrical growth  Factor ,1.3
    ninfL=25;% Cells in the boundary inflation layer
    ...
    %UPWIND ZONE
    %At which  index in the grid matrix start the plate?
    %nxSolid(1)- 1 %25

    for j=nxSolid(1)- 1:-1:1
        %if j<nxSolid(1) -9
        %    grthF_horizontal=1.1;
        %else
        %    grthF_horizontal=1.01;
        %end
        %new first step
        delta_y_1=(Ytest(ny,j+1)-Ytest(ny+1,j+1));
        %Assign as new y step
        Ytest(ny,j)=delta_y_1;
        %recompute inflation layers for the rest of the cells        
        %boundary 
        for i=ny-1:-1:ny-ninfL
             dy_inf=delta_y_1*(grthF)^(ny-i);%layer lenght
             Ytest(i,j)=Ytest(i+1,j) + dy_inf;
        end
        %Smooth transition
        %Calculating inflation layer parameters to fill the remaining
        %spaces in  y grid vector and have a smooth transition
